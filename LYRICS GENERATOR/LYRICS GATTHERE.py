...
spotify_user_id=''
# playlist containng songs fow which lyrics are collected
spotify_playlist_id=''
genius_key=''
...
#function cleans the lyrics removes [],(),''
def cleaner(son):
  son=son.strip()
  son=son.split('\n')
  waste=['', '[']
...
cleaned_lyrics=[]
ran=1
# storing the clean lyrics in a list
for i,j in enumerate(song_lyrics):
    try:
        cleaned_lyrics.append(cleaner(song_lyrics[i]))
    except:
        ran=ran+1
        continue
# writing the clean lyrics to a txt file
with open('rock.txt', 'w', encoding='utf-8') as file:
